...
if($_GET) {
  $name = $_GET['name'];
  $test = $_GET['test'];
  error_log("examples/node-programs/server.php GET: ip=$ip, agent=$agent, ref=$ref, name=$name, test=$test");
...
if($_POST) {
  $name = $_POST['name'];
  $test = $_POST['test'];
  error_log("examples/node-programs/server.php POST: ip=$ip, agent=$agent, ref=$ref, name=$name, test=$test");
