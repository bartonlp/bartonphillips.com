...
function getInfo(start=false) {
  ...
  query('web').then(data => {
    ...
    const grandTotal = formatMoney.format(gt);
...
`;
    ...
    let ar = [],
    byeValue = 0, orgValue = 0,
    curPrice, curChange, curPercent, curHigh, curLow, curUpdate,
    qty, byePrice, company, status,
    companyName, div, divYield, moving50, moving200, prevClose, divDate, divExDate, divValue, divTotal = 0;

    for(const[symbol,quote] of Object.entries(data.stock)) {
      ...
      moving50 = quote.moving50;
      moving200 = quote.moving200;
      divDate = quote.divDate;
      divExDate = quote.divExDate;
      ...
      if(symbol == 'DIA') {
        ...
        curHigh *= 100;
        curLow *= 100;
        let p = formatMoney.format(curPrice),
        high = formatMoney.format(curHigh),
        low = formatMoney.format(curLow),
        change = formatMoney.format(curChange),
        percent = curPercent;
    ...
    str += `<tfoot>\n<tr><th>Totals</th><th>${grandTotal}</th><th>${byeOrg}<br>${byeOrgDiff}</th>
           <th colspan='2'></th><th>${divTotal}</th><th></th>\n</tfoot>\n</table>`;
