...
<td>${qty}<br>${value}</td><td>${avVol}<br>${latestVolume}</td><td>${change}<br>${percent}</td><td>${status}</td></tr>`;
    ...
    <thead>
    <tr><th>Mutual</th><th>Price</th><th>Change %</th><th>Buy Price<br>%Diff</th><th>Qty<br>Value</th><th>Date</th></tr>
    </thead>
    <tbody>
`;
...
`;
      ...
      str += `
      <td>${mprice}</td><td>${mutPer[k]}</td><td>${buyprice}<br>${mbuyper}</td><td>${qty}<br>${value}</td><td>${date}</td><tr>
`;
    }
