...
if($_GET['page'] == "EndOfDay") {
  ...
  $str = "https://api.iex.cloud/v1/data/core/quote/DIA?token=$iex_token"; 
  $iex = json_decode(file_get_contents($str));
  foreach($iex as $v) {
    $dji = number_format(round(($v->latestPrice * 100), 2), 2);
    $djiDate = date("r", $v->latestUpdate / 1000);
  }
...
if($_POST['page'] == 'web') {
  ...
  foreach($iex as $v) {
    ...
    $djiDate = date("r", ($v->latestUpdate / 1000));
    $change = ($v->change * 100);
    $changePercent = ($v->changePercent * 100);
  }

  //error_log("stock-price-update.php: $dji, $djiDate, $change, $changePercent");
  ...
  //error_log("stock-price-quote.php: ". print_r($ar, true));
  ...
  while([$mutual, $mprice, $qty, $company] = $S->fetchrow('num')) {
    ...
    $mdate = $alpha['07. latest trading day'] ?? '';
    $close = $alpha['05. price'] ?? 0;
    $value = ($close * $qty) ?? '';
    $mutPer[$mutual] = $alpha['10. change percent'] ?? '';
