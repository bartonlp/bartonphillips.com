...
<thead>
<tr><th>Stock</th><th>Price</th><th>Av Price</th><th>Buy Price<br>% Diff</th><th>Qty<br>Value</th><th>Cur Vol</th><th>Change<br>% Change</th><th>Status</th></tr>
</thead>
<tbody>
`;
    ...
    for(const stocks in orgStock) {
      ...
      let avPrice, avVol, volume;

      curPrice = s.currPrice || 0;
      //curChange = s.change || 0;
      //curPercent = s.changePercent;
      curUpdate = new Date(s.currTime);
      ...
      console.log("stock: " + st + ", curPrice: " + curPrice + ", curUpdate: " + curUpdate + ", volume: " + volume);
...
<td></td><td>${orgPrice}<br>${orgPer}</td>
<td>${qty}<br>${value}</td><td>${volume}</td><td></td><td>${status}</td></tr>`;
    }
...
`;
      ...
      str += `
      <td>${mprice}</td><td></td><td>${buyprice}<br>${mbuyper}</td><td>${qty}<br>${value}</td><td>${date}</td><tr>
`;
    }
