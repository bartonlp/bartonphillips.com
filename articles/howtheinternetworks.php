...
<p>When we want to send a letter to someone, we write the letter and place it in an envelope. The
...
<p>We then put the letter in the mailbox. At some point, the mailman comes and collects the letter
  and drives to the local post office. Once it gets there, the mail is sorted. In
  ...
  placed on airplanes that took the mail to a major post office in the state or region where the
  recipient lived.</p>
...
<p>Even though the mail traveled via several transport mechanisms, it was at the routing end points
  that the letter's envelope was examined to see where it would next travel.</p>

<p>The envelop is analogous to the TCP/IP IP header which contains the sender's IP address and the
...
and faster optical fiber transmission lines, radio and satellites. Depending of how far away the
destination address is, different transmission media are used. The transmission media have no
knowledge of the sender or receiver address. All of that information (and more) is in packets in the
transmission media just like the letter inside a mail truck, or train or plane. Only at the
...
network to the client's computer and into the destination &quot;INBOX&quot; (in the case of email).
...
<p>The TCP/IP stack, as it is called, is made up of several layers. The top is the
<i>application</i> layer with protocals like SMTP, HTTP, FTP,etc. Next is the <i>transport</i> layer
-- UDP or TCP. This handles the sequencing and end point assignments (<a
...
layer is <i>Internet</i>  where the &quot;envelope&quot; is addressed with the sender's IP
address and the recipient's IP address.  The final layer is the <i>link</i> one where the
routing is done.</p>

<p>The TCP/IP stack does not concern itself with the final layer which is the <i>physical</i>.
...
<p>At every layer of the TCP/IP stack, the user's data is encapsulated in identifying headers and
  processed by specific software that understands that type of data. At the application layer, there
  are many different protocols and software but as we decend the stack, the choices narrow.</p>
...
and receiver is set up and the receiver tells the sender that it has received the message
correctly.  If there is a problem, the sender resends the information until it is received OK (or the
connection times out in which case an error is sent back to the initiating process.)</p>
...
1980s. IPv6 uses a 64-bit IP address which yields 1.844674407x10<sup>19</sup> addresses, somewhat
...
<p>At the <i>link</i> layer there are several routing protocols (ARP/InARP, NDP etc.).
   These are used by the routers at end points.</p>
...
from optical end-point to end-point through repeaters before being unbundled and turned back into
...
When an HTTP connection is made by a client browser, the Apache software will communicate with the
...
between 0 and 1,023. The inbound port is well-known to clients and is specified via <a
...
<p>There are many well-known ports that are used by standard TCP/IP protocals. For example, here are a
few very well-known ports: FTP: 20, SSH: 22, Telnet: 23, SMTP: 25, DNS: 53, HTTP: 80, POP3: 110, NTP:
...
<p>HTTP (Hypertext Transfer Protocal) is an <i>application</i> layer protocal. Using TCP/IP the
  client browser creates a <b>Request</b> and sends it to an HTTP host that is listening on well-known
  port 80. HTTP hosts can listen on other private posts which are known only to specific
  privilaged users. This is often done by client help systems and other services known to a specific
...
<p>The HTTP protocal has gone through a couple of revisions. The original HTTP/1.0 specified three
  ...
  DELETE, TRACE and CONNECT. By far the most use command is GET followed by POST and HEAD. The other
...
<b>Request</b> is sent to the host by the client and usually asks for a specific webpage. The
<b>Response</b> is sent back to the client from the host and if everything was successful, it has the
HTML of the webpage and a header.</p>

<p>The <b>Request</b> for a webpage looks like this:</p>
...
which is between 20 and 60 bytes and has the client's IP address and the host's IP address along with
...
'/howtheinternetwork.php'. The second line identifes the host. The server uses the 'Host:'
information to determine the virtual host that is being requested. The URI bartonphillips.org
resolves via DNS (Domain Name Service) to an IPv4 address: in this case 157.245.129.4. However,
157.245.129.4 also is the home of bartonphillips.com and several other
websites.  The rest of the lines tell the Apache server how to return the data.</p>
...
mentioned, 'howtheinternetworks.php'. Once the server finds the file, it processes the information
and creates a <b>Response</b> header and attaches the processed information in HTML format
...
<p>The web server host sends the <b>Response</b> information back to the client's IP address.  The
