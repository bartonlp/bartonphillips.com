...
function getinfo($value=null, $sql) {
  ...
  function trackerCallback2(&$desc) {
    ...
    $desc = preg_replace("~<tr><td>(.*?)</td><td>(.*?)</td>~", "<tr><td class='id'>$1</td><td class='ip'>$2</td>", $desc, -1, $cnt);
...
/*
 ...
 from $S->masterdb.tracker `;
    ...
    $.ajax({
      url: './findip.php',
      data: { page: 'find', sql: sql, where: where, and: 'and lasttime>current_date - interval 5 day', by: 'order by lasttime desc' },
      type: "post",
      success: function(data) {
        const newWindow = window.open("https://bartonphillips.com/Archive/findip.php", "_blank", "", true); 
