...
function getinfo($value=null, $sql) {
  ...
  // If the $ip form trackerCallback() is empty then nothing else will be valid!
  if(empty($ip) {
    error_log("findip2 ip empty: id=$id, line=". __LINE__);
    $botsTbl = $locstr = $geoTbl = $badTbl = $ip2info = "<h1>$id was not valid.</h1>";
  } else {
  ...
  EOF;
    ...
    // If ip is empty then api.ip2location.io uses the ip of the caller which in this case is my
    // Server.

    if($info->ip !== '192.241.132.229') {
...
// AJAX or POST. Find the information given by 'where', 'and', 'by' and 'sql'.
// This can be a POST from this page or an AJAX from the BroadcastChanel.
...
// START Validation. Check if valid. $ref must be from 'bartonphillips.com'

if(!str_contains($ref, "bartonphillips.com")) {
  echo <<<EOF
<h1>Authorization Denied</h1>
<p>Try <a href="https://bartonphillips.com/findip2.php">findip2.php</a>.</p>
EOF;
